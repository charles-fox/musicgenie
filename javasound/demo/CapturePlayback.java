/*
 * @(#)CapturePlayback.java	1.1.1.4  99/10/13
 *
 ...
 *
 * @version @(#)CapturePlayback.java	1.1.1.4 99/10/13
...
public class CapturePlayback extends JPanel implements ActionListener, ControlContext {
    ...
    AudioInputStream previousAudioInputStream;
    ...
    String errStr;
    double duration, previousDuration, seconds;

    LoadFrame loadFrame = new LoadFrame();
    boolean loadedByFile;
    ...
    public void open(javax.media.sound.midi.Receiver receiver) { }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (obj.equals(auB)) {
            saveToFile(textField.getText().trim(), AudioFileFormat.AU);
        } else if (obj.equals(aiffB)) {
            saveToFile(textField.getText().trim(), AudioFileFormat.AIFF);
        } else if (obj.equals(waveB)) {
            saveToFile(textField.getText().trim(), AudioFileFormat.WAVE);
            ...
            if (loadFrame.isShowing()) {
                ...
                loadFrame.show();
                previousAudioInputStream = audioInputStream;
    ...
    public void saveToFile(String name, Type type) {

        if (audioInputStream == null) {
            reportStatus("No loaded audio to save");
            return;
        } else if (loadedByFile) {
            loadFrame.createAudioInputStream();
        }
        ...
        try {
            ...
            AudioFileFormat[] fileFormats = AudioSystem.getAudioFileFormats(type , audioInputStream);

            if (fileFormats.length == 0) {
                System.out.println("Cannot save stream as type " + type);
            } else {
                if (AudioSystem.write(audioInputStream, fileFormats[0], file) == -1) {
        ...
        public void run() {
            ...
            while (thread != null) {
                try {
                    ...
                    int numFramesRemaining = numBytesRead / frameSizeInBytes;
                    while (numFramesRemaining > 0) {
                        numFramesRemaining -= line.write(data, 0, numFramesRemaining);
                    }
                } catch (Exception e) {
    ...
    class Capture implements Runnable {
        ...
        public void run() {
            ...
            long milliseconds = (long)((audioInputStream.getLength() * 1000) / format.getFrameRate());
    ...
    class FormatControls extends JPanel {
        ...
        JToggleButton linrB, ulawB, alawB, rate8B, rate11B, rate22B, rate44B;
        ...
        public FormatControls() {
            ...
            rate22B = addToggleButton(p2, sampleRateGroup, "22050", false);
            rate44B = addToggleButton(p2, sampleRateGroup, "44100", true);
            add(p2);
        ...
        public AudioFormat getFormat() {
            ...
            Type encoding = AudioFormat.ULAW;
            ...
            if (encString.equals("linear")) {
                if (signedString.equals("signed")) {
                    encoding = AudioFormat.PCM_SIGNED;
                } else {
                    encoding = AudioFormat.PCM_UNSIGNED;
                }
            } else if (encString.equals("alaw")) {
                encoding = AudioFormat.ALAW;
            }
        ...
        public void setFormat(AudioFormat format) {
            Type type = format.getEncoding();
            if (type == AudioFormat.ULAW) {
                ulawB.doClick();
            } else if (type == AudioFormat.ALAW) {
                alawB.doClick();
            } else if (type == AudioFormat.PCM_SIGNED) {
                linrB.doClick(); signB.doClick(); 
            } else if (type == AudioFormat.PCM_UNSIGNED) {
    ...
    class SamplingGraph extends JPanel implements Runnable {
        ...
        public void createWaveForm(byte[] audioBytes) {
            ...
            if (audioBytes == null) {
                try {
                    audioBytes = new byte[
                        (int) (audioInputStream.getLength()
                        * format.getFrameSize())];
        ...
        public void run() {
            ...
            while (thread != null) {
                if ((playback.line != null) && (playback.line.isOpen()) ) {
                    long milliseconds = (long)((playback.line.getPosition() * 1000) / playback.line.getFormat().getFrameRate());
                    seconds =  milliseconds / 1000.0;
                } else if ( (capture.line != null) && (capture.line.isActive()) ) {
                    long milliseconds = (long)((capture.line.getPosition() * 1000) / capture.line.getFormat().getFrameRate());
                    seconds =  milliseconds / 1000.0;
                }
    ...
    /**
     * Frame for loading an AU, WAV or AIFF file.
     */
    public class LoadFrame extends JFrame implements ActionListener {
        ...
        public LoadFrame() {
            super("Load File for Playback");
            addWindowListener(new WindowAdapter() {
                public void windowClosing(WindowEvent e) { }
            });
            JPanel panel = new JPanel(new BorderLayout());
            JPanel p1 = new JPanel();
            p1.add(new JLabel("AU, WAV or AIFF File :"));
            textField = new JTextField("audio/");
        ...
        public void createAudioInputStream() {
            ...
            if (file != null && file.isFile()) {
                try {
                    ...
                    long milliseconds = (long)((audioInputStream.getLength() * 1000) / audioInputStream.getFormat().getFrameRate());
                    duration = milliseconds / 1000.0;
                    auB.setEnabled(true);
                    aiffB.setEnabled(true);
                    waveB.setEnabled(true);
